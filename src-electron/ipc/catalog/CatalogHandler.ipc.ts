...
import { openChartWorkspace } from '../sng/ChartWorkspace.js'
...
export async function catalogUpdateChart(input: { id: number; updates: Partial<ChartRecord> }) {
	...
	try {
		// Update song.ini on disk (inside the workspace dir)
		await updateSongIni(ws.dir, input.updates)

		const { changed } = await ws.commit()
		if (changed) {
			// Rescan to pick up changes
			return await scanner.rescanChart(chart.path)
		}
		return db.getChart(input.id)
	} finally {
		await ws.discard()
	}
}
