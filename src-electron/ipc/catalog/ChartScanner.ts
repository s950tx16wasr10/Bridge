...
import { readSngFiles } from '../sng/SngReader.js'
...
class ChartScanner extends EventEmitter<ScannerEvents> {
	...
	private scanIdleResolvers: Array<() => void> = []
	...
	async scanLibraryPaths(libraryPaths: string[]): Promise<ScanResult> {
		...
		try {
			...
			this.scanning = false
			const resolvers = this.scanIdleResolvers
			this.scanIdleResolvers = []
	...
	/**
	 * @returns a promise that resolves when no full library scan is active.
	 */
	whenScanIdle(): Promise<void> {
		if (!this.scanning) {
			return Promise.resolve()
		}
		return new Promise(resolve => this.scanIdleResolvers.push(resolve))
	}
	...
	private async discoverChartFolders(rootPath: string): Promise<ChartFolder[]> {
		...
		const scanDir = async (dirPath: string, depth: number): Promise<void> => {
			...
			// Delete stray repack artifacts from a crash before the replace rename
			// (the commit never reported success, so roll-back-by-deletion is correct).
			// Fresh artifacts are skipped: a live commit's replacement file exists here
			// transiently, and deleting it would fail that commit with ENOENT.
			for (const entry of entries) {
				if (entry.isFile() && entry.name.toLowerCase().endsWith('.sng.bridge-new')) {
					await deleteStaleRepackArtifact(path.join(dirPath, entry.name))
				}
			}
	 ...
	 * (chart/ini/album contents loaded; video/oversized files become empty-data placeholders; parse errors return a partial list)
	 */
	private async getFilesFromSng(sngPath: string): Promise<{ fileName: string; data: Uint8Array }[]> {
		return readSngFiles(sngPath, {
			loadData: (fileName, fileSize) =>
				(hasChartExtension(fileName) || hasIniExtension(fileName) || hasAlbumName(fileName)) &&
	...
	async rescanChart(folderPath: string): Promise<ChartRecord | null> {
		...
		if (isSng) {
			// Delete a stray repack artifact from a crash before the replace rename
			await deleteStaleRepackArtifact(`${folderPath}.bridge-new`)
		} else {
			const entries = await fs.promises.readdir(folderPath, { withFileTypes: true })
			for (const entry of entries) {
				if (entry.isFile() && entry.name.toLowerCase().endsWith('.sng.bridge-new')) {
					await deleteStaleRepackArtifact(path.join(folderPath, entry.name))
				}
			}
			files = entries.filter(e => e.isFile() && !e.name.toLowerCase().endsWith('.sng.bridge-new')).map(e => e.name)
		}
...
/**
 * Removes a `.sng.bridge-new` repack artifact only when it is old enough to be a
 * crash leftover. A fresh artifact belongs to an in-flight commit's replace step.
 */
async function deleteStaleRepackArtifact(artifactPath: string): Promise<void> {
	try {
		...
		// Already gone or unreadable; the startup sweep is the backstop
	}
}
