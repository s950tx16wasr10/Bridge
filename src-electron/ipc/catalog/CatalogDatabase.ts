...
class CatalogDatabase {
	...
	/**
	 * Checks if any chart of each song exists in the library, matched by
	 * normalized (artist, name) pairs regardless of charter.
	 * Returns a map of `artist|name` (normalized) -> exists.
	 */
	checkSongsExist(songs: Array<{ artist: string; name: string }>): Map<string, boolean> {
		const result = new Map<string, boolean>()
		...
		songs.forEach(s => {
			result.set(`${normalize(s.artist)}|${normalize(s.name)}`, false)
		})

		const checkSongs = this.db.transaction(() => {
			this.db.exec(`
				CREATE TEMP TABLE IF NOT EXISTS temp_song_lookup (
					artist TEXT,
					name TEXT
				)
			`)
			this.db.exec('DELETE FROM temp_song_lookup')

			const insertStmt = this.db.prepare(
				'INSERT INTO temp_song_lookup (artist, name) VALUES (?, ?)'
			)
			for (const song of songs) {
				insertStmt.run(normalize(song.artist), normalize(song.name))
			}

			const matches = this.db.prepare(`
				SELECT LOWER(TRIM(c.artist)) as artist, LOWER(TRIM(c.name)) as name
				FROM charts c
				INNER JOIN temp_song_lookup t
					ON LOWER(TRIM(c.artist)) = t.artist
					AND LOWER(TRIM(c.name)) = t.name
			`).all() as Array<{ artist: string; name: string }>

			for (const match of matches) {
				result.set(`${match.artist || ''}|${match.name || ''}`, true)
			}
