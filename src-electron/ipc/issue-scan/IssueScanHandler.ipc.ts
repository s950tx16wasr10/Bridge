...
async function getFilesFromSng(sngPath: string) {
	...
	// Video and truncated files are omitted from the result entirely; parse errors reject
	return readSngFiles(sngPath, {
		onHeader: h => header = h,
		loadData: fileName => !hasVideoExtension(fileName) && !isFileTruncated(fileName),
		omitUnloaded: true,
	})
}
