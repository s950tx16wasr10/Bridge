...
class LyricsService extends EventEmitter<LyricsServiceEvents> {
	...
	async deleteLyrics(chartPath: string, chartType: 'mid' | 'chart' | 'sng' | null): Promise<{ success: boolean; error?: string }> {
		try {
			...
			if (chartType === 'mid' || !chartType) {
				// Removing the PART VOCALS track would require parsing and rebuilding the MIDI
				const midFile = files.find(f => f.toLowerCase() === 'notes.mid') ||
					files.find(f => f.toLowerCase().endsWith('.mid'))

				if (midFile) {
					return { success: false, error: 'Removing lyrics from .mid charts is not supported' }
				}
			}
	...
	async getAudioAsDataUrl(chartPath: string): Promise<{ dataUrl: string; vocalStartMs: number | null; hasVocalsTrack: boolean } | null> {
		try {
			if (chartPath.toLowerCase().endsWith('.sng')) {
				return await this.getAudioFromSngAsDataUrl(chartPath)
			}
	...
	/**
	 * Find the preferred audio file inside a .sng archive and return it as a data URL
	 * Vocal start detection is skipped for archives (ffmpeg needs a file on disk)
	 */
	private async getAudioFromSngAsDataUrl(sngPath: string): Promise<{ dataUrl: string; vocalStartMs: number | null; hasVocalsTrack: boolean } | null> {
		// List entry names from the header only (no file data is read)
		const header = await readSngHeader(sngPath)
		const names = header.fileMeta.map(f => f.filename)
		...
		for (const name of vocalNames) {
			const match = names.find(n => n.toLowerCase() === name)
			if (match) {
				audioFile = match
				hasVocalsTrack = true
				break
		...
		if (!audioFile) {
			const audioExtensions = ['.ogg', '.opus', '.mp3', '.wav', '.flac']
			audioFile = names.find(n => audioExtensions.includes(path.extname(n).toLowerCase())) ?? null
		}
		...
		// Load only the wanted file via a positional read
		const wanted = audioFile
		const files = await readSngEntries(sngPath, fileName => fileName === wanted)
		const entry = files.find(f => f.fileName === wanted)
		...
		return { dataUrl, vocalStartMs: null, hasVocalsTrack }
	}
