import { Component, ElementRef, HostBinding, OnInit, ViewChild, inject, signal } from '@angular/core'
import { ReactiveFormsModule, FormsModule, FormControl } from '@angular/forms'
import { NgClass } from '@angular/common'

import _ from 'lodash'
import { LastfmService } from 'src-angular/app/core/services/lastfm.service'
...
export class SettingsComponent implements OnInit {
	...
	lastfmUsername: FormControl<string>
	lastfmApiKey: FormControl<string>
	...
	lastfmTesting = signal(false)
	lastfmTestResult = signal<{ ok: boolean; message: string } | null>(null)
	...
	constructor() {
		...
		this.lastfmUsername = new FormControl<string>(ss.lastfmUsername, { nonNullable: true })
		this.lastfmUsername.valueChanges.subscribe(value => ss.lastfmUsername = value)
		this.lastfmApiKey = new FormControl<string>(ss.lastfmApiKey, { nonNullable: true })
		this.lastfmApiKey.valueChanges.subscribe(value => ss.lastfmApiKey = value)
	...
	async testLastfmConnection() {
		if (this.lastfmTesting()) { return }
		this.lastfmTesting.set(true)
		this.lastfmTestResult.set(null)
		const result = await this.lastfmService.validateUser(this.lastfmUsername.value.trim(), this.lastfmApiKey.value.trim())
		this.lastfmTesting.set(false)
		if (result.ok) {
			this.lastfmTestResult.set({ ok: true, message: `✓ ${result.username} — ${result.playcount.toLocaleString()} scrobbles` })
		} else {
			const messages: Record<string, string> = {
				badKey: 'Invalid API key.',
				unknownUser: `That user doesn't exist on Last.fm.`,
				private: `This user's listening data is private.`,
				rateLimited: 'Rate limited by Last.fm. Try again in a moment.',
				network: 'Failed to reach Last.fm. Check your connection.',
			}
			this.lastfmTestResult.set({ ok: false, message: messages[result.code] ?? result.message })
		}
	}
